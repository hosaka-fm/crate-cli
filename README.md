...
`2.1.0` (`GET /api/v2/openapi.json` — `info.version` pins the surface).

## Development

```sh
npm install && npm test        # builds, then runs unit + process-level contract tests
npm run gen:types              # regenerate src/generated/ from the live spec
```
